...
export default function Referral() {
    ...
    const referralCodes = useSelector(state => {console.log(state.campusAmbassador.user);return state.campusAmbassador.referralCodes});
    ...
    <div className="flex flex-col gap-6">
        {Object.entries(referralCodes).map(([competition, code]) => (
            <div 
                key={competition} 
                className="flex justify-between items-center border border-gray-200 p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow bg-gray-50"
            >
