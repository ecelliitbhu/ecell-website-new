...
const OpportunitiesPage = () => {
  ...
  >([]);
  const [opportunities, setOpportunities] = useState<OpportunityApplication[]>(
    []
  );
    ...
    try {
      ...
      if (studentId) {
        try {
          ...
          appliedPostIds = applications.map((app) => app.postId);
        } catch (error) {
      ...
      const availablePosts = posts.filter(
        (post) => !appliedPostIds.includes(post.id)
      );
    ...
    try {
      ...
      if (withdrawnApplication) {
        const opportunityToRestore = {
          ...
          applied: false,
          postedDate:
            withdrawnApplication.postedDate || new Date().toISOString(),
        };
