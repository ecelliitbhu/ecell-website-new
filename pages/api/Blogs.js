...
router.post("/api/blogs",[verifyLoggedin,verifyAdmin],async (request,response) =>  {
    await dbConnect();
    try {
        Blog.create({
...
router.get("/api/blogs",[verifyLoggedin,verifyAdmin],async (request,response) =>  {
    await dbConnect();
    try {
...
router.put("/api/blogs",[verifyLoggedin,verifyAdmin],async (request,response) =>  {
    await dbConnect();
    try {       
        let finder=await Blog.find({
            email : request.headers.email,
        ...
        }else{response.status(404).send("Blog not found")}
    } catch (error) {
        response.send("Some error happened")
    }
})

router.delete("/api/blogs",[verifyLoggedin,verifyAdmin],async (request,response) =>  {
    await dbConnect();
    try {
        let finder=await Blog.find({
            // email : request.headers.email,
