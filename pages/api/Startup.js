...
router.get("/api/startup/search/",[verifyLoggedin,verifyEmail],async (request,response) => {
    await dbConnect()
    try {
        let finder=await Startup.find(
            request.body
...
router.get("/api/startup/email/",[verifyLoggedin,verifyEmail],async (request,response) =>  {
    await dbConnect()
    try {
        let finder=await Startup.find({
...
router.get("/api/startup/id/",[verifyLoggedin,verifyEmail],async (request,response) =>  {
    await dbConnect()
    try {
        let finder=await Startup.find({
            email : request.headers.email,
