...
export const recruitersAPI = {
  ...
  getPending: async () => {
    const response = await fetch(`${BACKEND_URL}/recruiters/pending`);
    if (!response.ok) {
      throw new Error(`Failed to fetch pending recruiters: ${response.status}`);
    }
    return response.json();
  },

  verify: async (id) => {
    return fetch(`${BACKEND_URL}/recruiters/verify/${id}`, {
      method: "PUT",
    });
  },
};
